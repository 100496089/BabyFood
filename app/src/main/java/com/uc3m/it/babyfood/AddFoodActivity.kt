...
class AddFoodActivity : AppCompatActivity(){
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (mRowId == null) {
            ...
            mRowId = extras?.getLong(FoodRegisterAdapter.KEY_ROWID)
        }
        ...
        if (mRowId != null) { // si hay id es que lo que estamos haciendo es mdificar un registro, por eso fetchNote
            ...
            NameText!!.setText(
                note.getString(
                    note.getColumnIndexOrThrow(FoodRegisterAdapter.KEY_NAME)
                )
            )
            commentText!!.setText(
                note.getString(
    ...
    fun camera(v: View) {
            ...
            if (ubicacion != null) {
                ...
                intent.addFlags(Intent.FLAG_GRANT_READ_URI_PERMISSION or Intent.FLAG_GRANT_WRITE_URI_PERMISSION) //porque hay camaras que necesitan escribir
                intent.putExtra(MediaStore.EXTRA_OUTPUT, photoURI)// paso la ruta de la foto a la app de camara
    ...
    private fun obtenerUbicacionImagen(): File? {
        ...
        val fichero = File(
            directorioExterno.path + File.separator +
                    "IMG_" + timestamp + ".jpg"
        )
    ...
    override fun onActivityResult(requestCode: Int, resultCode: Int, data: Intent?) {
        ...
        if (requestCode == com.uc3m.it.babyfood.AddFoodActivity.Companion.CODIGO_HACER_FOTO) {
            if (resultCode != RESULT_CANCELED) {
                ...
                val miImagen = findViewById<View>(R.id.imagen) as ImageView
                val miBitmap = BitmapFactory.decodeFile(ubicacion!!.path)
    ...
    fun saveNote(view: View?) {
        ...
        if (mRowId == null) {
            val id = dbAdapter!!.createNote(name, comment, date) // si antes no habia la crea
            if (id > 0) {
                mRowId = id
            }
        } else {
            dbAdapter!!.updateNote(mRowId!!, name, comment, date) // y si no la
        }
        setResult(RESULT_OK)
        dbAdapter!!.close()
        val intent = Intent(
